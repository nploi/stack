...
void Game::menu() {
    ...
    cout << Color::color(Color::Code::BOLD);
    cout << Color::color(Color::Code::FG_DEFAULT);
    util::gotoxy(55, 11);
...
bool Game::removeExcess(Block &block) {
    ...
    if(block.getSize() < SIZE && block.getSize() == arrBlock[size - 1].getSize()) {
        counter++;
        if(counter >= 4) {
            int numLeft = 1, numRight = 1;
            if(SIZE - block.getSize() == 1) {
                if(score%2==0) {
                    numLeft = 0;
                } else {
                    numRight = 0;
